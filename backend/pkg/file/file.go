...
func SaveFile(r *http.Request, path string) (string, error) {
	// 2mb
...
func SaveFileWithName(r *http.Request, path string, fileName string) (string, error) {
	// 2mb
	err := r.ParseMultipartForm(1048576 * 2)
	if err != nil {
		return "", err
	...
	fileName = fmt.Sprintf("%s.%s", time.Now().UTC().Format("20060102150405") + "-" + fileName, strings.ReplaceAll(fileType, "image/", ""))
	dirPath := "./uploads/" + path
	err = os.MkdirAll(dirPath, 0755) // Create directory with appropriate permissions
	if err != nil {
		return "", err
